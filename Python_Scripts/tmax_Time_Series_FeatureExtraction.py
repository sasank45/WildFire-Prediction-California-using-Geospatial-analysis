...
base_tmax_dir = "/home/ubuntu/WildFire_Capstone/Data/tmax"

#output directory for .tif files
output_tmax_dir = "/home/ubuntu/WildFire_Capstone/Output/tmax"
...
for year in years:
    ...
    year_dir = os.path.join(base_tmax_dir, str(year))
    ...
    outpath = os.path.join(output_tmax_dir, "Time_Series_Features", str(year))
    ...
    if files:  # Check if there are any .tif files to process
        with gw.config.update(ref_image=files[0]):
            with gw.open(files, band_names=["tmax"]) as ds:
                print(ds)
